...
The public Apache vhost on `ethernetservers` routes L0 requests only to the browser inference coordinator (`127.0.0.1:18768`). Trusted engines must not be Apache hot standbys: they accept JSON timing, not the public task-ID/capability contract. During the approved Razer-only cutover, `/etc/systemd/system/babel-l0-swarm.service.d/backends.conf` sets `COORDINATOR_BACKEND_URLS=http://127.0.0.1:28767`. Razer must remain awake and connected; there is no trusted-server fallback. Homeserver (`127.0.0.1:18767`) is offline and excluded until its interrupted image build is completed and its new API is verified. Do not re-enable it merely because the host comes back.

The Mac backend (`127.0.0.1:38767`) remains detached, with its inference and reverse-tunnel LaunchAgents uninstalled. To restore it, deploy the current engine, run `./scripts/Install-MacService.sh` and `./scripts/Install-MacTunnel.sh` as the normal login user, and verify transcription plus JSON timing-based drafting through gateway port 38767. Only then add its URL to the coordinator's `COORDINATOR_BACKEND_URLS` override and restart the coordinator. Never add trusted engines directly to the public Apache balancer.
